        ...
        <div class="d-flex justify-center" v-for="data in this.roomList" >
          ...
          <v-card-title>{{ data.Room.roomName }}</v-card-title>
            ...
            <div class="justify-end d-flex">
              <v-btn @click="joinChat(data.Room.roomName)" class="ma-4" color="blue">Join</v-btn>
            </div>
          </v-card>
...
export default {
  ...
  async  fetchAllRooms()
    ...
    async joinChat(roomName) {
      await this.socket.emit('disconnectFromAllRooms');
      await this.socket.emit('new user', {username: this.username, room: roomName});
      await router.push("/word-game")
    },
